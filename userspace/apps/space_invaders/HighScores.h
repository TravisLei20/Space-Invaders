...
public:
  ...
  // into the highScores.
  HighScores(uint32_t score, std::string username);
...
public:
  ...
  // Initializes file path of executable, and locates or creates high_scores.txt
  void init();
  ...
  // file.
  std::vector<std::pair<std::string, uint32_t>> all_high_scores;
};
